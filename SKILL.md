...
description: 【中文名：跨行业通用可视化（数据看板生成器）】跨行业通用：把任意行业的明细数据（销售/运营/物流/医疗/教育/零售/外贸/餐饮/医美/酒店/医院/金融/航天军工/制造/文旅/电商/直播/宠物/快消/进出口/汽配/教培/...）做成「脱敏 + 可选密码加密（默认免密）」的单文件交互看板，无需写代码。12 类面板：KPI / 月度走势 / 维度排行 / 漏斗 / 热力 / 桑基 / 雷达 / 仪表盘 / 树图 / 拓扑 / 留存 / 人物画像 + 智能经营分析报告。地图：真实中国地图（省→市→区三级钻取）+ 世界地图（22 国州/省地图下钻）。**支持实时大屏**（--live URL 定时刷新全部图表）。全图点击联动 / 10 套主题切换 / 手机端适配 / 离线可用 / 数据太大用 --sample 采样。明确不适用：实时行情盘口/金融级机密/百万级大数据 SPA BI；这些请用专业工具。声明式 viz-schema 零代码适配。触发词：数据分析、数据可视化、报告生成、数据洞察、商业分析、BI、Excel 转可视化、自动生成报告、跨行业通用可视化、数据看板生成器、可视化、看板、图表、报表、数据大屏、实时大屏、实时数据看板、销售看板、收入分析、经营分析、行业看板、地图可视化、脱敏、加密看板、做个看板、dashboard。
metadata:
  author: WorkBuddy
  version: "0.9.4"
  agent_created: true
...
> ⚠️ **用前必看 · 30 秒决策**：本工具擅长【任意行业明细数据 → 一次性可视化汇报（单文件/脱敏/加密/地图钻取/实时大屏）】；
> **不适合**【实时行情盘口 / 金融级机密 / 百万级大数据 / 多页 SPA BI / 复杂工作流】——这些请用 Wind、FineBI、Tableau 等专业工具。

| 你的场景 | 用什么 |
|---|---|
| 销售/医疗/教育/金融/外贸…明细数据（几十~几万行），做汇报看板 | ✅ **本工具** |
| 实时盘口 / 高频行情（K线/撮合） | Wind / 同花顺 / 东方财富 |
| 金融级机密 / 合规审查级数据 | 专业加密方案 |
| 百万级大数据实时分析 | ClickHouse + 专业 BI |
| 多页 SPA / 复杂 BI 应用 | FineBI / Tableau / Power BI |
...
本工具**不是万能的**。下列场景**请直接用专业工具**，本工具做不了也做不好：

- ❌ **实时行情盘口/高频交易**（K 线、盘口、撮合）→ 用 Wind / 同花顺 / 东方财富
- ❌ **金融级机密**（涉及数千万合规审查、医疗患者隐私立法）→ 用专业加密方案
- ❌ **百万级大数据实时分析** → 用 ClickHouse + 离线数仓 + 专业 BI（Power BI / Tableau / 网易有数）
- ❌ **多页 SPA/路由型 BI 应用** → 用 FineBI / Tableau / 微软 Power BI
- ❌ **复杂工作流/审批系统** → 用飞书多维表格 / 钉钉审批 / 专业 OA

✅ **本工具擅长**：任意行业明细数据 → 一次性可视化汇报（销售/医疗/教育/金融/外贸/物流/航天军工/制造/文旅/...），单文件双击即用，加密可控。需要"看一眼数据"而非"建一套系统"时，是首选。

> 注：地图的世界边界从内部缓存加载（如遇网络不稳可能降级为列表展示，可重试或升级 assets/countries 下的边界文件）。
