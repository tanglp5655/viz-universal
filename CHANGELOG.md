...
## [0.9.10] - 2026-08-18 — 修复建议数字 bug + 12 行业多维复核通过

### 修复
- **建议数字 bug**：build_decisions 中 pair 取值错误（months[-2] 误作元组），导致"激活下滑签单人"建议显示 0→0；修复为 (旧月, 新月) 元组。

### 复核（12 行业 × 多维）
- 12/12 行业全部自动生成决策/定制建议/亮点/注意点/科目变化/开局，无崩溃；
- 数字交叉验证：建议引用数据与实际一致（激活签单人/扩科占比）；
- 降级场景：无维度极简数据不崩溃、合理降级；
- HTML 渲染完整性 10/10（一句话结论/完整性预检/亮点/注意点/科目变化/开局/洞察/收益/定制建议/SVG 图）；
- 回归 90/90。

## [0.9.9] - 2026-08-18 — 行动建议业务定制+数据驱动（对齐专业报告深度）

### 新增
- **业务定制建议**（build_decisions 接入 _dp）：每条建议自动从报告数据中提取——下滑签单人/扩科放量/校区新增/双引擎延续等，自动生成"具体动作"含实际数字（"黄佳敏 1.1万→5440（-50%），目标回到 3.6万/月"）+ "预期收益"含估算（"¥3.6万"）。
- **建议表 5 列**（HTML/MD）：建议主题 / 具体动作 / 负责人 / 期限 / 预期收益——对齐 agentos 报告建议表叙事深度。
- 阈值微调：爆发/下滑阈值放宽（≥1.4 倍爆发 / ≤0.6 倍下滑）更敏感。
- 实测薛弈：自动得出"激活黄佳敏 + 扩科策略加码"业务定制建议（与人工报告结论一致）。
- 回归 90/90。

## [0.9.8] - 2026-08-18 — 报告配图（内嵌 SVG：趋势/条形/环形，离线无依赖）

### 新增（对齐 agentos 报告"图文并茂"）
- **纯 SVG 图表生成器**（离线、无外部库）：组合图（柱=金额+折线=笔数）、横向条形（科目/签单人/老师）、环形图（客单段）。
- 报告配图：一、月度走势（趋势组合图）；交叉结构（科目条形图）；签单人表现（条形图）；客单结构（环形图）；服务人员结构（条形图）——文字+图表穿插。
- 回归 90/90。

## [0.9.7] - 2026-08-18 — 报告故事化（本月亮点/风险注意点/科目排名变化/最新月开局）

### 新增（对齐薛弈专业经营报告叙事结构）
- **本月亮点**：自动提炼 2-3 条（双签单引擎/扩科放量/高客单结构/营收高增/新增量）。
- **风险注意点**：自动提炼 2-4 条（科目量大价低/服务人员依赖/签单集中/个人下滑/数据窗口）。
- **主营结构变化**：完整月对 Top3 科目金额环比 + 排名跃升（如"围棋低段班 ¥9,248→¥58,679 上升4位"）。
- **最新月开局解读**：最新月笔数/金额 + 构成 + 来源 + 节奏推算。
- 实测薛弈文化：与专业报告结论一致（低段班跃居第一/数学第二/英语量大价低/8月开局4单¥9,080）。
- 回归 90/90。

## [0.9.6] - 2026-08-17 — 报告风格升级：文字叙述为主 + 专业商务视觉

### 变更
- **文字为主、图表为辅**（对齐专业经营报告）：深度分析从表格堆砌改为叙述段落——交叉结构/增长驱动/签单人表现/客单结构/服务人员结构/扩科专项全部文字化（如"签单人表现：陈颖 6月2.1万→7月11.5万（+449% 爆发）…"）。
- **专业商务视觉**（参考薛弈扫描版报告）：浅灰纸张背景 + 白色纸张卡片、蓝色章节标题、KPI 大数字卡、红（预警）/绿（结论）/蓝（决策）信息框、打印无边框。
- 回归 90/90。

## [0.9.5] - 2026-08-17 — 数据口径溯源 + 建议预期收益列

### 新增
- **数据口径溯源**：anonymize 记录来源文件/数据库，报告顶部显示"合并 N 份来源（文件名）→ 去重后 X 条明细"——对齐专业报告的溯源习惯。
- **建议"预期收益"列**：每条行动清单增加预期收益（如"消除数据窗口假象，避免误判方向""锁定增长/下滑主因，聚焦资源投放"），HTML/MD 双格式。
- 回归 90/90。

## [0.9.4] - 2026-08-17 — 全行业通用性验证 + 主营结构 label 动态化

### 变更
- **4 行业实测**（餐饮/医院/金融/外贸）：一句话结论/客单结构/签单人月度贡献/驱动分解/扩科专项全部自动适配（语境自动切换：大堂经理/赵医生/理财B/外贸二队）。
- **主营结构 label 动态化**：洞察不再硬编码"课程结构"，改用 dims 中 c 的实际 label（科目/项目/产品/收费项目…）。
- 回归 90/90。

## [0.9.3] - 2026-08-17 — 深度对等版（一句话结论/客单结构/老师结构/校区/个人状态/扩科专项）

### 新增（对齐专业深度报告，全行业通用——字段驱动）
- **一句话结论**（报告最前）：营收+环比+驱动+Top签单人+扩科占比+数据完整性，一眼看懂。
- **客单结构**：P50/P75/均值 + 低/中/高三段分布（任何有金额的数据都适用）。
- **服务人员结构**：teacher 列 Top5 + 占比 + 兜底标签检测（兼职/外包）。
- **场所/门店结构**：campus 列 Top5 + 最新完整月"新增场所"检测。
- **签单人个人状态**：Top 签单人按月标记（爆发/下滑/平稳），如"陈颖 6月2.1万→7月11.5万，+449% 爆发"。
- **扩科/加购专项**：src 含"扩科"类词时独立分析（笔数/金额/占比/环比/科目 Top）。
- 实测薛弈文化 195 行：一句话结论+客单结构+扩科专项+个人状态全部自动生成，与专业人工报告实质对等。
- 回归 90/90。

## [0.9.2] - 2026-08-17 — 深度分析引擎（交叉/驱动分解/签单人矩阵/结构洞察）

### 新增
- **交叉分析**：来源 × 项目 Top6 组合（金额+占比），识别主力组合（如 老生→围棋高段班 17.4%）。
- **环比驱动分解**：最近完整月 Δ 按 来源/课程/签单人 拆解 Top3 贡献——回答"增长/下滑是谁驱动的"（如 7 月 +140% = 老生+71k + 数学课程+39k + 陈颖+94k）。
- **签单人月度矩阵**：Top3 签单人 × 各月金额，看个人产能与波动。
- **结构洞察**（自动生成）：课程集中度、来源结构（老生/新生）、新生客单价 vs 整体、签单单点依赖、课次单价。
- HTML 报告新增「六~九章 深度分析」，Markdown 简报新增「深度分析」段。
- 实测薛弈文化 5-7 月 195 行：自动得出"7 月增长由老生续费+数学/英语课程扩张+陈颖爆发驱动"。
- 回归 90/90。

## [0.9.1] - 2026-08-17 — 数据完整性预检（拿下 DA"数据窗口假象"高地）

### 新增
- **月度完整性预检**：统计每月覆盖天数，最新月覆盖 <15 天 或 笔数 <上月 30% → 标记"不完整月"。
- **环比修正**：不完整月不参与趋势判断，自动用最近完整月重算环比（如教培 8 月仅 4 单 → 改用 7 月 vs 6 月 +125%）。
- **三处输出**：HTML 报告顶部 ⚠️ 完整性预检红条；决策简报开头明确"数据窗口假象"警告；行动清单第一条变为"拉取 X 月全月数据"。
- 实测教培 203 行：自动识别"8 月仅覆盖至 04 日（4 笔）→ -96% 环比为假象"，与竞品 Data Analysis 人工判断结论一致。
- 回归 90/90。

## [0.9.0] - 2026-08-17 — 智能决策简报（补齐分析维度，对齐竞品 Data Analysis）

### 新增
- **决策简报引擎**（report.py build_decisions）：从数据自动生成 2-4 个决策问题——趋势可持续性 / 结构集中风险 / 区域依赖 / 数据质量。
- **每个决策含 6 要素**：决策问题 → 简短答案 → 证据 → **置信度（高/中/低+理由）** → **注意事项（什么会改变结论）** → **行动清单（负责人/期限）**——对齐竞品 Data Analysis 决策简报模板。
- **统计增强**：区域/维度集中度 CR1、缺失率、样本期覆盖率自动参与置信度评级（>=6 月高 / 3-5 月中 / <3 低；缺失>10% 降档）。
- **输出三格式**：HTML 报告新增「📌 决策简报」章节；`--report-md` 输出 Markdown 决策简报（可粘贴汇报）；PDF 同步。
- 回归 90/90；实测真实订单 1175 行：自动得出"营收下滑是否可持续 / 四川 81% 区域集中 / 数据采集完善"三决策，与竞品人工简报结论一致。
